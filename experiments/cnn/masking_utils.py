...
TODO: decide where this module belongs organically in terms of logic.
    E.g. it can be under utils or pruners.
...
def get_wrapped_model(model: "nn.Module") -> "nn.Module":
    ...
    assert not next(
        model.parameters()
    ).is_cuda, "Model shouldn't be on CUDA, \
...
class WrappedLayer(nn.Module):
    ...
    def __init__(self, layer, *args, **kwargs):
        ...
        self.register_buffer(
            "_weight_mask", torch.ones_like(self._layer.weight).requires_grad_(False)
        )
    ...
    def _build_pruned_copy(param, new_mask, prev_copy=None):
        ...
        *modify_masks* method is called (typically by a pruner
        ...
        """
        if param is None or new_mask is None:
            return None
        # print("@@@@@@@@@ making a new copy of size ", new_mask.mean())
        if prev_copy is None:
            return param.data * (1 - new_mask)
        # print("@esdfdsf probably shouldn't be here")
    ...
    def _mask_setter(self, new_mask, name="weight"):
        ...
        if self._copy_pruned:
            setattr(
                self,
                f"_{name}_data_copy",
                self._build_pruned_copy(param, new_mask, param_data_copy),
            )
    ...
    def set_previous_masks(self):
        self._prev_weight_mask = torch.clone(self.weight_mask)
        self._prev_bias_mask = None
        if self.bias_mask is not None:
            self._prev_bias_mask = torch.clone(self.bias_mask)

    @property
    def prev_weight_mask(self):
        return self._prev_weight_mask

    @property
    def prev_bias_mask(self):
        return self._prev_bias_mask
    ...
    def layer_sparsity(self):
        ...
        """
        return (
            num_zero(self.weight_mask),
            num_zero(self.bias_mask),
    ...
    def revert_pruned(self):
        """
        Reverts the pruned weights to their pre-pruning values.
        USE_CASE: when reintroducing the neurons.
        """
        if not self._copy_pruned:
            raise ValueError("No weights maintained to revert")
        if self._weight_data_copy == None:
            return
        if self.bias_mask is not None:
            revert_bias_mask = ~self.bias_mask.byte()
            self.bias.data[revert_bias_mask] = self._bias_data_copy[revert_bias_mask]
            setattr(self, "_bias_mask", torch.ones_like(getattr(self, "_bias_mask")))
            # self.bias_mask = torch.ones_like(self.bias_mask)
        revert_weight_mask = ~self.weight_mask.bool()
        # print("size of revert_weight_mask: ", revert_weight_mask.float().mean())
        # print("before restoring weight", self.weight.data.abs().mean())
        # print(f"weight_data_copy: {self._weight_data_copy.abs().mean()} {self._weight_data_copy[revert_weight_mask].float().abs().mean()}")
        self.weight.data[revert_weight_mask] = self._weight_data_copy[
            revert_weight_mask
        ]
        # print("after restoring weight", self.weight.data.abs().mean())
        # self.weight_mask = torch.ones_like(self.weight_mask)
        setattr(self, "_weight_mask", torch.ones_like(getattr(self, "_weight_mask")))
        self._weight_data_copy = None
        self._bias_data_copy = None
    ...
    def mask_grad(self):
        ...
        USE_CASE: can be invoked by user? pruner? (TODO: Discussion)
        """
    ...
    def copy_pruned(self, enable: bool):
        ...
        if not enable:
            raise ValueError("Enable not on!")
            self._weight_data_copy = None
