...
class ImageNetCLSLMDBDataset(LMDBDataset):
    def __init__(
        self, root, split="train", class_id=0, transform=None, target_transform=None
    ):
    ...
    def __getitem__(self, index):
        return super().__getitem__(
            1 + index + self.class_split_dict[str(self.class_id - 1)]
        )

    def __len__(self):
        return (
            self.class_split_dict[str(self.class_id)]
            - self.class_split_dict[str(self.class_id - 1)]
        )
...
class COOPLMDBDataset(LMDBDataset):
    def __init__(self, root, split="train", transform=None) -> None:
        ...
        idx_to_class = OrderedDict(
            sorted({s[-2]: s[-1] for s in split_file["test"]}.items())
        )
